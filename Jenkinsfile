pipeline {
    ...
    stages {
        stage('Clone') {
            steps {
                ...
                slackSend(channel: '#jenkins-notifications', color: 'good', message: "Cloning repository...")
        ...
        stage('Build') {
            steps {
                script {
                    ...
                    slackSend(channel: '#jenkins-notifications', color: 'good', message: "Building the application...")
        ...
        stage('Deploy - Run docker container') {
            steps {
                script {
                    slackSend(channel: '#jenkins-notifications', color: 'good', message: "Cleaning up old container...")
                    
                    sh "docker stop ${IMAGE_NAME} || true"
                    
                    sh "docker rm ${IMAGE_NAME} || true"

                    slackSend(channel: '#jenkins-notifications', color: 'good', message: "Running Docker container from image: ${IMAGE_NAME}")
    ...
    post {
        ...
        always {
            script {
                slackSend(channel: '#jenkins-notifications', color: 'good', message: "Post-build steps complete.")
            }
        }
        success {
           slackSend(channel: '#jenkins-notifications', color: 'good', message: "Deployment Successful: ${IMAGE_NAME} - Build ${IMAGE_NAME}")
        }
        failure {
            slackSend(channel: '#jenkins-notifications', color: 'danger', message: "Deployment Failed: ${IMAGE_NAME} - Build ${IMAGE_NAME}")
        }
    }
}